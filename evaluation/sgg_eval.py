...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container,
    ...
    local_container[
        'obj_scores'] = prediction.o_scores
    # about pan_seg masks
    prediction_masks = [mask_util.decode(prediction.masks[i]) for i in range(len(prediction.masks))]
    local_container['gt_masks'] = groundtruth.masks

    # local_container['pred_masks'] = prediction.masks
    local_container['pred_masks'] = prediction_masks
